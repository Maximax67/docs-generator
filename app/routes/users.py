...
from app.models.users import AllUsersResponse, UserDocumentsResponse
...
async def update_user(telegram_id: int, user_update: User):
...
async def get_saved_variables(telegram_id: int):
    user = await User.find_one(User.telegram_id == telegram_id)
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
async def update_saved_variables(telegram_id: int, saved_variables: Dict[str, str]):
...
async def update_saved_variables(telegram_id: int):
...
async def delete_saved_variable(telegram_id: int, variable: str):
...
async def update_saved_variable(telegram_id: int, variable: str, value: str):
    validate_variable_name(variable)
    validate_variable_value(value)
    ...
    existing_user = await User.get_pymongo_collection().find_one(
        {"telegram_id": telegram_id},
        {"_id": 1},
    )
